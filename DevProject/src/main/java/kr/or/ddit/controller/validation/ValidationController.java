...
public class ValidationController {
	...
	public String registerValidateFormResult(@Validated Member member, BindingResult result) {
	...
	/*
	 * 2. 입력값 검증 결과
	 * 	- 입력값 검증 대상의 도메인 클래스 식후에 BindingResult를 정의한다.
	 * 
	...
	/*
	 * 3. 입력값 검증 규칙
	 * - 입력값 검증 규칙은 Bean Vaildation이 제공하는 제약 어노테이션으로 설정한다.
	 * 
	 * 		검사 규칙은 크게 다음 세가지로 분류할 수 있다.
	 * 		- Bean Validaion 표준 제약 어노테이션
	 * 		- 서드파티에서 구현한 제약 어노테이션(서드파티란 제3사에서 만든...)
	 * 		- 직접 구현한 제약 어노테이션
	 * 
	 ...
	 * 		@NotEmpty				: 글자 수나 컬렉션이 요소 개수를 검사한다.
	 ...
	 * 		- 테스트 URL : /validation/registerValidaionorm02로 요청해서 테스트를 진행해봅시다!
	 * 
	 * 4. 중첩된 자바빈즈 입력값 검증
	 * - 중첩된 자바빈즈와 자바빈즈의 컬렉션에서 정의한 프로퍼터에 대해 입력값 검증을 할 때는 @Vaild를 지정한다.
	 * 
	 ...
	 *  3) Card 크래스에도 validation을 설정한다.
	 */
	
	@RequestMapping(value = "/registerValidationForm03", method = RequestMethod.GET)
	public String registerValidaionForm03(Model model) {
		log.info("registerValidaionForm03() 실행...!");
		model.addAttribute("member", new Member());
	...
	public String registerValidaionResult3(@Validated Member member, BindingResult result) {
		log.info("registerValidaionResult3() 실행...!");
