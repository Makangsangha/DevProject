...
public class MybatisController {
	 	...
	 	- 마이바티스 기능을 활용하여 매핑 파일을 적절하게 정의하면 일대다 관계 테이블 매핑을 쉽게 처리할 수 있다.
	 		...
	 		1) 게시판 구현 설명
				...
				- 여기까지 확인
	 			...
	 			- 회원 수정 가능 컨트롤러 메소드 만들기(crudMemberModify:post)
	 			- 회원 수정 기능 서비스 인터페이스 메소드 만들기
	 			- 회원 수정 기능 서비스 클래스 메소드 만들기
	 			- 회원 수정 기능 Mapper 인터페이스 메소드 만드기
	 			- 회원 수정 기능 Mapper xml 쿼리 만들기
	 			- 회원 수정 완료 페이지 만들기 (위에서 이미 만듬)
	 			- 여기까지 확인
	...
	/*
	 	...
	 	1. Tiles란?
	 		...
	 		우리는 그러한 공통 부분들을 분리하여 반복적을 컴포넌트들을 사용하는게 아니라 공통적인 부분을 한번만 가져다 쓰고
	 		변화하는 부분에 대해서만 동적으로 변환해 페이지를 관리 할 수 있어야할 것입니다.
	 		이렇게, header/footer/menu 등 공통적인 소스를 분리하여 한 화면에서 동적으로 레이아웃을 한 곳에 배치 하여
	 	  ...
	 	  ** 모듈화를 진행할 페이지
	 	  ...
	 	  - footer.jsp (footer 영역에서 사용할 페이지입니다)
	 	  	...
	 	  	그외에 다양한 영역의 페이지는 구현하고자 하는 시나리오를 바탕으로 페이지가 구성될때 추가적으로 레이아웃을 영역을 분리하여
	 	  	작성하면 됩니다.
