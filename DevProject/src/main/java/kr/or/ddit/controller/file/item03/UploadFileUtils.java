...
public class UploadFileUtils {
	...
	public static String uploadFile(String uploadPath, String originalName, byte[] fileData) throws IOException {
		...
		FileCopyUtils.copy(fileData, target); // 위에서 만들어진 경로와 파이명을 가지고 파일 복사를 진행한다.
	...
	private static void makeThumbnail(String uploadPath, String path, String fileName) throws IOException {
		...
		// Mehotd.AUTOMATIC : 최소 시간 내에 가장 잘 보이는 이미지를 얻기 위한 사용 방식
		...
		String formathName = fileName.substring(fileName.lastIndexOf(".") + 1);
		ImageIO.write(destImg, formathName.toUpperCase(), newFile); // 's_'가 붙은 썸네일 이미지를 만든다.
	}
