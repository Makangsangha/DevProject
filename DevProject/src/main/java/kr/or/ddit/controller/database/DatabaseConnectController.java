...
public class DatabaseConnectController {
	 ...
	 * 			query
	 * 			- ResultSetExtractorm, RowCallbackHandler와 함께 조회할 때 사용한다.
	 *			update
	 *			- 데이터르 변경하는 SQL을 실행할 때 사용한다.
	 *		
	 *		2. 스프링 JDBC 설정
	 *			
	 *			spring-jdbc 의존관계 설정
	 ...
	 *		3. 게시판 구현
	 *			
	 *			** 기본적인 JDBC를 이용한 디비 설계는 DAO란에서 sql 쿼리가 노출되어 개발하는 방식이다보니 우리가 중급프로젝트 때 또는 JSP에서
	 *			preparestatement방식의 데이터베이스 연동과 다를바 없는 형태로, 게시판 구현을 진행하지 않고 내용만 간략히 보고 넘어가도록 한다.
	 *		
	 ...
	 *  
	 *  # JPA 관련 어노테이션
	 *  자바빈즈 클래스 객체가 곧 테이블 구조와 같은 형태이기 때문에 , VO클래스 내 @Entity어노테이션을 통해 해당 객체가 테이블과 같은 Entity임으 
	 *  설정하고 @Table어노테이션으로 테이브명을 설정해주면 VO클래스는 데이터베이스의 생성되어 있는 테이블과 연결되기 위한 준비가 된다.
	 *  ex)
	 *  	@Entity
	 *  	@Table(name="board")
	 *  	public class Board{...
	 *  그리고 각 필드 (멤버변수)에 @Column(name="board_no") 어노테이션으로 테이블 컬럼과 매핑 설정을 한다.
	 *  설정된 Entity들을 이용하여 jap객체를 통해서 함수를 호출하면(persistm, find 등등) 호출된 함수를 통해 Spring Data JPA가
	 *  자동으로 Entity를 분석하여 함수에 따른 쿼리를 자동생성하여 결과를 실행, 조회, 수정 등을 처리해준다.
	 *	기존 SQL 쿼리를 이용하여 서비스를 처리할때에 필드가 추가되면 필드와 관련되어 있는 기능, sql, 추가된 필드 등을 수정해야하는 번거로움이
	 ...
	 *	데이터베이스와 통신하기 때문입니다.)
	 *
	 *  JPA : JPA구현체인 hibernate를 이용하여 사용합니다. 그러기 위해선 관련 의존관계를 등록 후 사용해야합니다.
	 ...
	 *  품질이 떨어진다는 문제가 있습니다. 그만큼 설계라인의 정확성이 필요하고 여러 방면에서 정확한 검증이 제대로 끝나지 않으면 적용하기가 까다롭습니다.
	 *  
	 *  
	 *  --------------------------------------
	 *  [용어]
	 ...
	 *  	- 저는 ibatis를 사용했습니다ㅡ 저는 mybatis를 사용했습니다 와 같은 대답으 하시면 됩니다.
	 *  --------------------------------------
	 *  
	 *  7. 마이바티스
	 *  - SQL과 자바 객체를 매핑하는 아이디어로 개발된 데이터베이스 접근용 프레임워크다.
	 *  
	 *  마이바티스 관련 내용은 12장에서 더 자세하게 아고 넘어가도록 합시다!
	 *  	
	 *  	1) 의존 관계 정의
	 ...
	 *  
	 *  	3) 마이바티스 설정
	 *  	- webapp/WEB-INK/mybatisAlias/mybatisAlias.xml 설정
	 *  
	 *  	4) 마이바티스 구현
	 *  	- mapper 패키지안에 Mapper 파일 구성(.xml)
	 *  	- 예제로 mapper패지키 안에 BoardMapper.xml을 구성하여 테스트를 진행합니다!
	 *  	
	 */
}
