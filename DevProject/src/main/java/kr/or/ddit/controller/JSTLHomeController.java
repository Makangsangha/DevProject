...
public class JSTLHomeController {
	 ...
	 * 		<c:param>				|	파라미터를 지정한다.
	 * 
	 * 		[ taglib 지시자 사용 ::: <%@ taglib url="http://java.sun.com/jsp/jstl/core" prefix="c" %>]
	 * 
	 * 		2) fmt태그 라이브러리
	 * 
	 * 				요소			|		설명
	 ...
	 * 
	 * 		[ taglib 지시자 사용 ::: <%@ taglib uri="http://java.sum.com/jsp/jstl/fmt" prefix="fmt" %>]
	 * 
	 * 		3) function 태그 라이브러리
	 * 
	 * 				요소			|		설명
	 ...
	 * 		<fn:endsWith>			|	지정한 문자열이 끝나는지 판단한다.
	 ...
	 *  	------------------------------------------------------
	 *  	value		|	Object		|	출력할 값
	 *  	escapseXml	|	boolean		|	특수 문자를 변환할지의 여부
	 *  	default		|	Object		|	value의 결과값이 null인 경우 출력할 값(대신출력됨)
	 *  	
	 *  	2) <c:set>
	 *  	- JSP에서 사용할 변수를 설정한다.
	 *  	속성		|	타입		|	설명
	 ...
	 *  	target		|	Object		|	프로퍼터 값을 설정할 객체 지칭
	 *  	property	|	String		|	프로퍼터 이름
	 *  
	 *  	3) <c:remove>
	 *  	- 설정한 변수를 제거한다.
	 *  	속성		|	타입		|	설명
	 ...
	 *  	------------------------------------------------------
	 *  	test		|	boolean		|	검사 조건
	 *  	var			|	String		|	검사 조건의 계산 결과값을 저장한 EL 변수
	 *  
	 *   	6) <c:choose>
	 *   	- 여러 조건을 처리할 때 사용한다.
	 *   
	 ...
	 *  	
	 *  	8) <c:otherwise>
	 *  	- <c:when>	요소에서 지정한 조건에 모두 일치하리 않을 때 처리할 내용을 구현한다. <c:choose> 요소에서 사용한다.
	 *  
	 *  	9) <c:forEach>
	 ...
	 *  	- 절대 URL : 완전한 URL이다.
	 *  	- 상대 URL :
	 *  		>	웹 애플리케이션 내에서의 절대 경로 : '/'로 시작하는 경로 (ex_ /board/list.jsp)
	 *  		>	현재 JSP에 대한 상대경로 : '/'로 시작하지 않는다. (ex_ ../board/list.jsp)
	 *  	웹 애플리케이션 내에서의 절대 경로를 사용할 실제로 생성되는 URL은 컨텍스트 경로를 포함한다.
	 *  
	 *    	13) <c:redirect>
	 *    	- 지정한 URL에 리다이렉트한다.
	 *  	속성		|	타입		|	설명
	...
	public String home0101(Model model) {
		...
		member.setUserId("<p>hongks<>&%0101</p>");
	...
	public String home0102(Model model) {
		Member member = new Member();
		member.setUserId("<p>hongks<>&%0102</p>");
		model.addAttribute("member", member);
	...
	public String home0103(Model model) {
		Member member = new Member();
		member.setUserId(null);	// 기존의 값이 셋팅되어 있으므로 null로 셋팅
		model.addAttribute("member", member);
	...
	// JSP에서 사용될 변수 memberId를 설정하여 사용하고
	// 태그의 몸체를 값으로 사용한다.
	...
	public String home0710(Model model) {
		Member member = new Member();
		String[] hobbyArray = {"Music", "Movie" };
		member.setHobbyArray(hobbyArray);
		model.addAttribute("member", member);
