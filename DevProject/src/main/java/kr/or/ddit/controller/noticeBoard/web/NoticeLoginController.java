...
//			@RequestParam(name="stat", required = false, defaultValue = "0") int stat,
			Model model) {
		// 메세지 처리를 쿼리 스트림에 특정 키값을 넘겨서 요청을 처리할 메소드에서  처리 할 수 있다. (방법1)
//		if(stat == 1) {
	...
	public String signup(
		...
		if(errors.size() > 0) {
			...
			if(result.equals(ServiceResult.OK)) {
				ra.addFlashAttribute("message","회원가입을 완료하였습니다!");	// redirect할 때의 일회성 메세지
				goPage = "redirect:/notice/login.do?staf=1";
			}else {
				model.addAttribute("bodyText", "register-page");
