...
public class JSPHomeController {
	 ...
	 * 
	 * 1. 지시자
	 * - 지시자(directive)는 JSP페이지에 대한 설정 정보를 지정할 때 사용.
	 * - JSP가 제공하는 지시자에는 page지시자, taglib지시자, include 지시자로 세가지가 있다. 
	 * 
	 * 		1) page 지시자
	 ...
	 * 
	 * 4. 표현언어(EL)
	 * - JSP는 EL(Expression Language)이라는 표현 언어를 사용해 값을 참조, 출력, 연산을 할 수 있다.
	 * - EL식은 ${...} 또는 #{...} 형식으로 작성한다.
	 * 
	 * # EL을 사용하여 객체를 조회하는 방법은 다음과 같다.
	 * - 자바빈즈 프로퍼티를 조회하는 경우 "속성명.프로퍼티명"을 지정한다.
	 * - List나 배열 요소를 조회하는 경우 "속성명[요소 위치]"를 지정한다.
	 ...
	 * 		   연산자		|		설명
	 * 		──────────────────────────────────────────────────────────────────────────────────
	 * 		&&(and)		|	두 피연산자 모두 true이면 bool true를 반환하고, 그렇지 않으면 false로 반환된다.
