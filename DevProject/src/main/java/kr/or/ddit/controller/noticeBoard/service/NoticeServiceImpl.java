...
public class NoticeServiceImpl implements INoticeService {
	...
	public ServiceResult insertNotice(HttpServletRequest req, NoticeVO noticeVO) {
		...
		if(status > 0) {
			...
			try {
				tst.sendGet("최강 깜직이 문기", noticeVO.getBoTitle());
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public ServiceResult updateNotice(HttpServletRequest req, NoticeVO noticeVO) {
		...
		if(status > 0) {	// 성공
			...
			try {
				...
				if(delNoticeNo != null) {
					// 3) 삭제를 원하는 파일 번호를 하나하나씩 데이터베이스로 던져서 파일 정보를 얻어 온 다음에
	...
	public ServiceResult deleteNotice(HttpServletRequest req, int boNo) {
		...
		if(status > 0) {
			...
			List<NoticeFileVO> noticeFileList = noticeVO.getNoticeFileList();
			// 파일이 있든 없든 삭제 처리를 해준다.
			if(noticeFileList != null && noticeFileList.size() > 0) {
				
				// D:\A_teaching\6_JspSpring\02.SPRING2\workspace_spring2\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\DevProject\resources\notice\24
				// e7974076-6ad0-47c8-974d-0019d590c7d3_wallpaperbetter_(1)_(1).jpg
				// '/'를 기준으로 잘라진다.
		...
		if(folder.exists()) {
			...
			for(int i = 0; i < fileList.length; i++) {
				if(fileList[i].isFile()) {	// 폴더안의 있는 파일이 파일 일때
					// 폴더 안에 파일을 차례대로 삭제
					fileList[i].delete();
				}else {						// 폴더안에 있는 파일이 폴더 일때 재귀함수 호출(폴더안으로 들어가라고)
	...
	public ServiceResult profileUpdate(HttpServletRequest req, DDITMemberVO memberVO) {
		...
		// 사용자가 수정한 프로필 이미지 정보에 따라서 프로필 이미지 정보 값으 설정 후 memberVO에 셋팅해서 전달한다.
		String uploadPath = req.getServletContext().getRealPath("/resources/profile");
		File file = new File(uploadPath);
		if(!file.exists()) {
			file.mkdirs();
