...
public class JSPFmttagController {
	 ...
	 * 
	 * 		** type 속성 : number일 경우 숫자 형식으로, parcent일 경우 % 형식으로, currency일 경우 통화 형식으로 출력한다.
	 ...
	 * 		var			|	String			|	파싱한 결과를 저장할 변수 이름을 지정한다.
	 * 
	 * 		** type 속성 : time, date, both 중 한가지 값을 가지 수 있으며 기본값은 date이다.
	 * 		dateStyle 속성 : default, short, medium, long, full 중 한가지 값을 가질 수 있으며, 기본값을 default이다.
	...
	// 1) type속성을 지칭하거나 pattern 속성을 지칭하여 숫자를 형식화한다.
	...
	// patter 속성을 사용하여 직접 사용할 서식을 지정한다.
