...
public class ExceptionController {
	/*
		...
		1. 예외처리
				...
				하지만, 페이지에 애플리케이션 서버의 내부 정보가 일반 사용자들에게 노출되어 프레임워크에 보안 취약점을 노린 공격을 받을 수 있다.
				이런 점을 고려하여 최대한 직접 예외를 처리하여 사용자가 정의한 에러 페이지를 표시하게 해야한다.
				...
				- B. 수정 화면 생성하고 뷰 파이에서 수정 진행 할때 예외 발생
				...
				2-1) 예외 발생 상황
					...
					- 등록할 때 제목에 빈 값을 입력하여 유효성 검증 예외 발생
						...
						> [예외 상황 만들 위치]
							...
							- vo.Board title 피드에 @NotBlank 설정
					...
					- 존재하지 않는 게시물을 조회할 때 사용자가 정의한 예외 바생
						> Http Status 500
						> [예외 상황 만들 위치]
							kr.or.ddit.service.impl.BoardServiceImpl read() 수정
						
					- 존재하지 않는 페이지 URL 요청 시 예외 바생
						> Http Status 404
						> [요청 URL]
							/crud/board/retry
		
		3. 상태 코드 사용한 에러 페이지 설정
			- 웹 커테이너 설정 파이 (web.xml)의 <error-code> 요소에 상태 코드를 설정하고 <location> 요소에 이동 대상 페이지를 지정한다.
			
				예외 처리 방법
				- 웹 컨테이너 서정(web.xml)
					> <error-page><error-page> 400 코드 서정
					> <error-page><error-page> 404 코드 서정
					> <error-page><error-page> 500 코드 서정
					
				- 처리할 수 있는 예외
					> 등록할 때 제목에 빈 값을 입력하여 유효성 검증 예외 발생
					> 수정 화면 생성하고 수정 진행 할 때 예외 발생
					> 삭제 할 때 매핑 파일에서 예외 발생
		...
		4. 예외 타입 사용한 에러 페이지 설정
			- 웹 컨테이너 설정 파일(web.xml)이 <exception-type> 요소에 예외 타입을 설정하고 <location> 요소에 이동 대상 페이지르 지정한다.
		...
		- @ExceptionHandle와 @ControllerAdvice를 이용하여 처리한다.
			...
			- @ExceptionHandler 어노테이션은 괄호안에 설정한 예외 타입을 해당 매소드가 처리한다는 것을 의미한다.
		...
		9. 입력값 검증 예외 처리
		- @Valiadted 어노테이션을 사용하면 Bean Validation의 유효성 검증 매커니즘을 이용할 수 있다.
