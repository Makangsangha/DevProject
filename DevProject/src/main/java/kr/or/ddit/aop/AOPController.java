...
public class AOPController {
	 ...
	 * 	그러던 어느날, 성희 팀장님을 통해서 '반장님, 지금 개발중인 서비스 처리 속도좀 로그로 남겨주세요!' 라는 부탁을 받습니다.
	 * 	반장 정수학생은 부탁받은 요구사항을 이행하기 위해서 본인이 만들고 있는 서비스 로직에서 처리 속도를 찍어볼 메소드를 개발해 처리 속도가 잘 찍히는 걸 확인합니다.
	 *  기분이 좋은 정수학생은 팀장님께 다려갑니다. '팀장님! 기능 작업 완료했습니다!' 라고 컨펌을 받습니다.
	 *  성희 팀장님은 아주 긍정적인 검토안을 정수학생한테 전달하면서 위 서비스 전체에도 각 처리속도를 찍어주세요 라고 다시 부탁합니다.
	 *  정수 학생은 본인이 만들어 낸 메소드를 각 기능별 서비스 로직에 하나하나씩 약 2만개쯤 넣을쯤에 의문을 갖습니다..
	 *  
	 *  	'서비스 로직에서 제일 중요한 로직은 보내의 기능이 제일 중요하고 지금 내가 작성하는 로직은 옵션(부가 기능)이 추가되는게 아니려나?
	 *  	'그럼 이걸 하나의 묶음으로는 처리가 불가능한건가?'
	 *  
	 *  하는 생각을 하게 됩니다. 그렇게 정수학생은 위와같은 고민을 해결하기 위해서 구글링을 시작합니다.
	 *  
	 *  여기서, 시간을 측정하고 권한을 체크하는 등의 기능을 옵션과 같은 부가기능으로 일종의 인프라 로직이라고 하는데, 이 인프라 로직은 애프리케이션 전 영역에서
	 *  나타날 수 있고, 중복코드를 만들어 내 개발의 효율성을 저하시키고 유지보수가 힘들어 질 수 있습니다.
	 *  
	 *  이러한 인프라 로직은 아래처럼 하나의 관심사를가질 수 있는데 이런 관심사들의 중복이 횡단으로 나타나는데, 이것으 가르켜 '횡단 관심사(Cross=Cutting Concern)'라고 함.
	 *  
	 *  	┌────────────────────────────────────────────────────────────────────────────────┐
	 ...
	 *  
	 *  	이런 횡단 관심사를 통해서 프로그ㅐ밍하는 것이 AOP라고 한다.
	 *  
	 *  		** 간단하게 맛보기
	 *  		- Aspect(어스팩트) : AOP의 단위가 되는 횡단 관심사
	 ...
	 *  		- @Componet : @Aspect와 짝꿍. component-scan시 "저 여기 있어요~ 여기 봐주세요!" 라는 의미
	 *  		- JoinPoint : 어드바이스(부가기능)가 적용될 수 있는 위치
	 *  		- Advice : 어떤 부가기능(메소드 실행 시작 전, 메소드 실행 후 등)을 언제(삼겹살을 굽기 전, 삼겹살을 다 먹은 후 등) 사용할지 정의
	 *  			* 언제?
	 *  			> Before : 조인포인트 전에 실행.(삼겹살을 굽기 전에)
	 *  			> After : 조인 포인트에서 처리가 완료된 후 실행.(삼겹살을 굽고 먹은 후 실행)
	 *  			> After Returning : 조인 포인트가 정상적으로 종료 후 실행
	 *  			> After Throing : 조인 포인트에서 예외 바생 시 실행, 예외가 발생안되면 실행 안함
	 *  			> Around : 조인 포인트 전후에 실행(삼겹살을 굽기 전과 먹은 후 실행)
	 *  
	 *  AOP(관점 지향프로그램(Aspect Oriented Programming))
	 ...
	 *  
	 *  	- 횡단 관심사의 분리
	 *  	> 어플리케이션을 개발할 때 횡단 관심사에 해당하는 부분을 분리해서 한 곳을 모으는 것을 의미한다.
	 *  
	 *  	1-2) AOP 개발 순서
	 ...
	 *  			기본적인 보안적용은 스프링 시큐리티에서 사용하고, 트랜잭션 관리는 @Transactional 어노테이션을 활용한 트랜잭션에서 사용하고,
	 *  			예외처리는 xml설정을 통한 예외 처리에서 사용하도록 합니다.
	 *  
	 *  	1-4) AOP 관련 용어
	 *  
	 *  		- 어스팩트(Aspect)
	 ...
	 *  			> 어디에 적용할 것인지 결정, 메소드? 객체생성자? 필드접근자? 등등
	 *  		- 어드바이스(Advice)
	 ...
	 *  			> 예외가 바생하지 않고 정상적으로 종료하면 실행되지 않는다.
	 *  		- Around
	 *  			> 조인 포인트 전후에 실행된다.
	 *  
	 ...
	 *  		------------------------------------------
	 *  
	 *  	2-2) 포인트컷 표현식에 사용되는 와이드카드
	 *  		
	 *  			와일드카드		|	설명
	 *  		-----------------------------------------------------------------------------------------------------------------------------
	 *  				*			|	임의의 패키기 1개 계층을 의미하거나 임의의 인수 1개를 의미한다.	
	 ...
	 *  		public void startLog(JoinPoin jp){
	 ...
	 *   
	 *    		AOP는 서비스내의 비즈니스 로직을 가로채서 컨트롤을 하는데, 이때 프록시라는 녀석을 이용하여 해당 기능을 실행하기 전 후 또는 예외 시에
	 ...
	 *    				프록시 여부를 확인하도록 합니다. (인터페이스/클래스 기반의 프록시 여부가 true/false로 추력 및 확인)
	 *    			
	 */
	...
	/*
	 * 8. 메소드 정보 획득
	 * - @Before 어노테이션 붙은 메소드는 joinPoin라는 매개변수를 통해 실행 중인 메소드의 정보를 구할 수 있다.
